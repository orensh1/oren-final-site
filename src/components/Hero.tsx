...
const Hero: React.FC = () => {
  ...
  return (
    <div ref={containerRef} className="relative min-h-screen w-full flex flex-col justify-center overflow-hidden bg-[#050505] text-white">
      
      {/* 1. Background Texture & Aurora */}
      
      {/* Noise Overlay */}
      <div className="absolute inset-0 z-[5] opacity-[0.05] pointer-events-none mix-blend-overlay"
...
const StatCard = ({ number, label, delay }: { number: string, label: string, delay: number }) => (
    <motion.div 
        ...
        className="relative group overflow-hidden rounded-xl border border-white/20 bg-white/[0.03] backdrop-blur-md p-6 hover:bg-white/[0.08] transition-all duration-500"
    >
